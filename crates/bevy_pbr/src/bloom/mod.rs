...
impl Plugin for BloomPlugin {
    fn build(&self, app: &mut bevy_app::App) {
        ...
        let bloom_node = BloomNode::default();
...
impl Node for BloomNode {
    fn input(&self) -> Vec<SlotInfo> {
        vec![SlotInfo::new(Self::IN_HDR, SlotType::TextureView)]
    }
    ...
    fn run(
        ...
        let camera_3d = match extracted_cameras.entities.get(CameraPlugin::CAMERA_3D) {
            Some(entity) => entity,
            None => return Ok(()),
        };
